...
def test_model_stack_trajectory_sampler_raises_for_model_without_trajectory_sampler() -> None:
    model0 = QuadraticMeanAndRBFKernel()
    model0.kernel = (
        gpflow.kernels.RBF()
    )  # need a gpflow kernel object for random feature decompositions
    model1 = QuadraticMeanAndRBFKernel()
    model1.kernel = (
        gpflow.kernels.RBF()
    )  # need a gpflow kernel object for random feature decompositions
    with pytest.raises(ValueError):
        HasTrajectorySamplerModelStack((model0, 1), (model1, 1))


def test_model_stack_trajectory_sampler_sample_is_deterministic() -> None:
    x = tf.zeros([10, 3], dtype=tf.float64)
    y1 = tf.ones([10, 1], dtype=tf.float64)
    y2 = tf.ones([10, 1], dtype=tf.float64)

    model1 = GaussianProcessRegression(gpr_model(x, y1))
    model2 = GaussianProcessRegression(gpr_model(x, y2))

    traj_model_stack = HasTrajectorySamplerModelStack((model1, 1), (model2, 1))
    test_x = tf.random.normal(shape=(20, 10, 4, 3), dtype=tf.float64)
    sample1 = traj_model_stack.eval_on_trajectory(test_x)
    sample2 = traj_model_stack.eval_on_trajectory(test_x)
    tf.debugging.assert_equal(sample1, sample2)
