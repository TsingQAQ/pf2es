...
from trieste.utils.sampler import QuasiMonteCarloNormalSampler
...
def test_batch_reparametrization_sampler_samples_approximate_mean_and_covariance() -> None:
    ...
    samples = BatchReparametrizationSampler(sample_size, model).sample(xs, qMC=True)
...
def test_batch_reparametrization_sampler_samples_are_continuous() -> None:
    ...
    npt.assert_array_less(tf.abs(sampler.sample(xs + 1e-20, qMC=True) - sampler.sample(xs, qMC=True)), 1e-20)


def test_batch_reparametrization_sampler_samples_are_repeatable() -> None:
    sampler = BatchReparametrizationSampler(100, _dim_two_gp())
    xs = tf.random.uniform([3, 5, 7, 2], dtype=tf.float64)
    npt.assert_allclose(sampler.sample(xs, qMC=True), sampler.sample(xs, qMC=True))
...
def test_batch_reparametrization_sampler_samples_are_distinct_for_new_instances() -> None:
    ...
    xs = tf.random.uniform([3, 5, 7, 2], dtype=tf.float64)
    npt.assert_array_less(1e-9, tf.abs(sampler2.sample(xs, qMC=True) - sampler1.sample(xs, qMC=True)))
...
@random_seed
def test_qmc_sampler() -> None:
    samples = QuasiMonteCarloNormalSampler(5).sample(10000)
    # samples = tf.random.normal(shape=(10000, 5))
    samples_mean = tf.reduce_mean(samples, axis=-2)
    samples_covariance = tfp.stats.covariance(samples, sample_axis=-2, event_axis=-1)

    npt.assert_allclose(samples_mean, tf.zeros_like(samples_mean), atol=3e-2, rtol=1)
    npt.assert_allclose(samples_covariance, tf.eye(num_rows=5), atol=1e-2, rtol=1)


@random_seed
def test_batch_reparametrization_sampler_samples_qmc_better_than_mc_on_approximating_mean_and_covariance() -> None:
    model = _dim_two_gp()
    sample_size = 128
    leading_dims = [3]
    batch_size = 4
    xs = tf.random.uniform(leading_dims + [batch_size, 2], maxval=1.0, dtype=tf.float64)
    mc_samples = BatchReparametrizationSampler(sample_size, model).sample(xs, qMC=False)

    samples_mean = tf.reduce_mean(mc_samples, axis=-3)
    samples_covariance = tf.transpose(
        tfp.stats.covariance(mc_samples, sample_axis=-3, event_axis=-2), [0, 3, 1, 2]
    )

    qmc_samples = BatchReparametrizationSampler(sample_size, model).sample(xs, qMC=True)

    qmc_samples_mean = tf.reduce_mean(qmc_samples, axis=-3)
    qmc_samples_covariance = tf.transpose(
        tfp.stats.covariance(qmc_samples, sample_axis=-3, event_axis=-2), [0, 3, 1, 2]
    )

    model_mean, model_cov = model.predict_joint(xs)

    tf.debugging.assert_greater_equal(tf.norm(samples_mean - model_mean), tf.norm(qmc_samples_mean - model_mean))
    tf.debugging.assert_greater_equal(tf.norm(samples_covariance - model_cov), tf.norm(qmc_samples_covariance - model_cov))
