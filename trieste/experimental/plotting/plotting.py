...
from ...acquisition.multi_objective.dominance import non_dominated
from ...types import TensorType
from ...utils import to_numpy
from ...acquisition.multi_objective.dominance import non_dominated
from ...acquisition.multi_objective.partition import prepare_default_non_dominated_partition_bounds, \
...
def view_2D_function_in_contourf(
    func,
    var_range,
    matplot_inst=None,
    show=True,
    ...
    map_var_range=None,
    kwargs_for_data_points=None,
    colorbar=False,
    **kwargs,
):
    '''
    :param var_range: [(lb, ub), (lb, ub), ..., (lb, ub)]
    '''
    if kwargs_for_data_points is not None:
        assert isinstance(kwargs_for_data_points, dict)
    else:
        kwargs_for_data_points = {}
    if matplot_inst is None:
        # TODO: may need improvement
        import matplotlib.pyplot as matplot_inst
    if kwargs.get("log_space"):
        x_1 = np.logspace(*var_range[0], plot_fidelity)
        x_2 = np.logspace(*var_range[-1], plot_fidelity)
    else:
    ...
    # TODO: meshgrid support
    # _z_data = func(x_grid).reshape(len(x_1), len(x_2))
    # 2019/10/29 changed to
    try:
        ...
        # _z_data = np.array([func(serial_x) for serial_x in x_grid])
        _z_data = np.array([func(serial_x) for serial_x in x_grid]).reshape(len(x_1), len(x_2))
    # _z_data = np.array([func(np.array([[_x, _y]])) for _x in x_1 for _y in x_2]).reshape(len(x_1), len(x_2))

    if map_var_range is not None:
        from PyOptimize.metamodel.utils.normallize import Invers_Scale_Parameters, Scale_Params

        x_1, x_2 = Invers_Scale_Parameters(
            Scale_Params(np.row_stack((x_1, x_2)).T, np.asarray(var_range)), map_var_range
        ).T
    if kwargs.get("normalize"):
        from matplotlib.colors import Normalize

        normalize = Normalize(vmin=0, vmax=1)
        cntr = matplot_inst.contourf(
            x_1,
            x_2,
            _z_data,
            kwargs.pop("color_number", 15),
            alpha=0.75,
    ...
    if kwargs.get("levels"):
        C = matplot_inst.contour(
            x_1,
            x_2,
            _z_data,
            kwargs.pop("line_number", 0),
            colors=kwargs.get("color", "white"),
            linewidths=kwargs.get("linewidths", 1.5),
            levels=kwargs.get("levels"),
        )
        matplot_inst.clabel(C, inline=True, fontsize=10)
    matplot_inst.title("{}".format(kwargs.get("title", "")))
    if kwargs.get("hide_ticks"):
        matplot_inst.xticks(())
        matplot_inst.yticks(())
    if plot_data_points is not None:
        assert (
            isinstance(plot_data_points, np.ndarray) == True
        ), "plot_data_points must be a numpy array"
        xs = plot_data_points
        assert xs.shape[-1] == 2, "data points format must be n * 2(feature) format!"
        matplot_inst.scatter(*xs.T, **kwargs_for_data_points)
    plt.xlim(var_range[0])
    plt.ylim(var_range[1])
    if colorbar:
...
def view_2D_function_in_contour(
    func,
    var_range,
    matplot_inst=None,
    show=True,
...
def plot_pf_dominated_region_2d(pf: TensorType, reference_point: TensorType, ax, facealpha: float=0.1):
    """
    Plot the Pareto front
    """
    lb, ub = prepare_default_dominated_partition_bounds(reference=reference_point, observations=pf)
    if ax is None:
        fig, ax = plt.subplots()
...
def plot_pf_dominated_region_boarder_2d(pf: TensorType, upper_bound: TensorType, ax, color=None, linewidth=None):
    """
    ...
    for i in range(sorted_pf.shape[0]):
        if color is not None:
            default_color = color
        if i == 0:
            ax.plot((sorted_pf[i][0], sorted_pf[i][0]), (sorted_pf[i][1], upper_bound[-1]),
                    color=color, linewidth=linewidth)
            if i != sorted_pf.shape[0] - 1:
                ax.plot((sorted_pf[i][0], sorted_pf[i+1][0]), (sorted_pf[i][1], sorted_pf[i][1]),
                        color=color, linewidth=linewidth)
            else:
                ax.plot((sorted_pf[i][0], upper_bound[0]), (sorted_pf[i][1], sorted_pf[i][1]),
                        color=color, linewidth=linewidth)
