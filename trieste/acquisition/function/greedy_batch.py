...
def _generate_fantasized_model(
    ...
    # we add manually that if model is TrainableHasTrajectoryAndPredictJointReparamModelStack
    if type(model) == TrainableHasTrajectoryAndPredictJointReparamModelStack:
        ...
        return TrainableHasTrajectoryAndPredictJointReparamModelStack(*fmods)
    if isinstance(model, ModelStack):
        observations = tf.split(fantasized_data.observations, model._event_sizes, axis=-1)
        fmods = []
        for mod, obs, event_size in zip(model._models, observations, model._event_sizes):
...
class _fantasized_model(
    ...
    def trajectory_sampler(self):
        # We hard code here
        assert isinstance(self._model, HasTrajectorySampler)
        # set fantasized data to model training data
        # copy model
        copied_model = deepcopy(self._model)
        # augment model with fantasized_data
        # assume no scaling happens!
        model_data = Dataset(copied_model.model.data[0].value(), copied_model.model.data[1].value())
        copied_model.update(
            model_data
            + Dataset(self._fantasized_query_points.value(), self._fantasized_observations.value())
        )
        return copied_model.trajectory_sampler()

    def reparam_sampler(self: ProbabilisticModelType, num_samples: int):
        # We set this only to mitigate error in PF2ES, it is not used anywhere in reality
