...
def prepare_default_non_dominated_partition_bounds(
    ...
    remove_augmentation_part: bool = False,
    ...
    :param remove_augmentation_part if set to True, the augmentation part in the non-dominated region
        will be removed
    ...
    if averaging_partition is False:
        if not remove_augmentation_part:
        ...
        else:  # we do the partition without augmentation part
            # we need to move extrema point of each output to reference point
            # observations = modify_observations_to_utilize_dominate_space_partition_strategy(observations, reference)
            inverse_lb, inverse_ub = prepare_default_dominated_partition_bounds(
                -observations, reference=-anti_reference
            )
            return -inverse_ub, -inverse_lb
    else:  # we return both
        assert tf.shape(observations)[-1] == 2
        with_aug_partion = ExactPartition2dNonDominated(observations).partition_bounds(
            anti_reference, reference
        )
        ...
        without_aug_partition = (-inverse_ub, -inverse_lb)
        return with_aug_partion, without_aug_partition




# TODO: Write test for this
def prepare_default_dominated_partition_bounds(
    observations: TensorType, reference: TensorType
) -> tuple[TensorType, TensorType]:
    """
    Prepare the default dominated partition boundary for acquisition function usage.
    ...
    if observations.shape[-1] > 2:
        return HypervolumeBoxDecompositionIncrementalDominated(
            observations, reference
        ).partition_bounds()
    elif observations.shape[-1] == 2:
        return ExactPartition2dDominated(observations).partition_bounds(
            tf.constant([-1e100] * observations.shape[-1], dtype=observations.dtype), reference
        )
    else:
...
class _BoundIndexPartition:
    ...
    def partition_bounds(
        ...
        tf.debugging.assert_greater_equal(
            reference,
            self.front,
            message=f"observations: {self.front} containing at least one value above"
            f"reference point {reference}, try specify a larger "
            "reference point.",
        )
...
class ExactPartition2dDominated(_BoundIndexPartition):
    def __init__(self, front: TensorType):
        """
        :param front: non-dominated pareto front
        """
    ...
    def _get_bound_index(self) -> _BoundedVolumes:
        # Compute the cells covering the *dominated* region for 2 dimension case
        # this assumes the Pareto set has been sorted in ascending order on the first
        # objective, which implies the second objective is sorted in descending order
        len_front, number_of_objectives = self.front.shape
        range_ = tf.range(len_front)[:, None]
        #  Note the difference: now the upper bound is actually fixed at worst point
        lower_result = tf.concat(
            [range_, range_], axis=-1
        )  # indices of lower bound (x_incies, y_indices)
        upper_result = tf.concat(  # indices of upper bound  (x_incies, y_indices)
            [range_ + 1, tf.ones(shape=(len_front, 1), dtype=tf.int32) * len_front], axis=-1
        )

        return _BoundedVolumes(lower_result, upper_result)

    def partition_bounds(
        self, anti_reference: [TensorType, None], reference: TensorType
    ) -> tuple[TensorType, TensorType]:
        """
        get partitioned hypercell bounds
        :param anti_reference: no use
        :param reference the worst point to be used
        """
        ...
        pseudo_pfront = tf.concat((self.front, reference[None]), axis=0)
        N = tf.shape(self._bounds.upper_idx)[0]
        D = tf.shape(self._bounds.upper_idx)[1]
        idx = tf.tile(tf.range(D), (N,))
