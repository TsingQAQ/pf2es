...
def automatic_optimizer_selector(
    ...
    if isinstance(space, DiscreteSearchSpace):
        ...
        if num_runs is None:
            num_runs = tf.minimum(NUM_RUNS_DIM * tf.shape(space.lower)[-1], 100)
...
def generate_continuous_optimizer(
    ...
    def optimize_continuous(
        ...
        candidates = space.sample(tf.minimum(num_initial_samples, 5000))[:, None, :]  # [num_initial_samples, 1, D]
        # candidates = space.sample(5000)[:, None, :]  # [num_initial_samples, 1, D]
        tiled_candidates = tf.tile(candidates, [1, V, 1])  # [num_initial_samples, V, D]
        # print('start batch random input')
        start = time.time()
        target_func_values = target_func(tiled_candidates)  # [num_samples, V]
        end = time.time()
        # print(f'Batch Random Input takes: {end - start}')
        ...
        optimizer_args = {"options": {"maxiter": 50}}
        ...
        # end_time = time.time()
        # print(f'Parallel L-BFGS-B takes: {end_time - start_time}')
...
def _perform_parallel_continuous_optimization(
    ...
    while True:
        ...
        # Batch evaluate query `x`s from all children. 这里是主要的, 对目标函数的Batch query 都在这里进行
        ...
        # print(f'batch l-bfgs-b gradient: {np_batch_dy_dx}')
        # print(f'batch l-bfgs-b gradient norm: {tf.linalg.norm(np_batch_dy_dx, axis=-1)}')
        # norm_candx.append(tf.linalg.norm(np_batch_dy_dx, axis=-1).numpy())
    ...
    # np.savetxt('norm_candx.txt', np.asarray(norm_candx))
