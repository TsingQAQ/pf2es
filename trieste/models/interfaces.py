...
class ModelStack(ProbabilisticModel, Generic[ProbabilisticModelType]):
    ...
    **Note:** Only supports vector outputs (i.e. with event shape [E]). Outputs for any two models
    ...
    def independent_sample(self, query_points: TensorType, num_samples: int) -> TensorType:
        """
        This is used to generate samples by ignoring the correlation between all the data
        """
        _means, _vars_ = zip(*[model.model.predict_f(query_points) for model in self._models])
        mean, var = tf.concat(_means, axis=-1), tf.concat(_vars_, axis=-1)
        var = tf.clip_by_value(var, 0.0, var.dtype.max)
        return tfd.Normal(loc=mean, scale=tf.sqrt(var)).sample(num_samples)
...
class TrainableModelStack(ModelStack[TrainableProbabilisticModel], TrainableProbabilisticModel):
    ...
    def optimize(self, dataset: Dataset, randomize_model_hyperparam: bool = True) -> None:
        ...
        for model, obs in zip(self._models, observations):
            model.optimize(Dataset(dataset.query_points, obs), randomize_model_hyperparam=randomize_model_hyperparam)
...
class HasTrajectorySamplerModelStack(ModelStack[HasTrajectorySampler]):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._trajectory_samples: Optional[list] = []
...
class TrainableHasTrajectoryAndPredictJointReparamModelStack(
    ...
    # TODO: Implement a sample method
