...
from .wrapper_model.transformer import Standardization
...
def build_standardized_gpr(
    ...
    :return: A :class:`~gpflow.models.GPR` model.
    """
    data = Dataset(data.query_points, Standardization().update_forward(data.observations))
    empirical_mean, empirical_variance, _ = _get_data_stats(data)
...
def _set_gaussian_likelihood_variance(
    ...
    # here we follow the suggestion of #603 by changing the lower bound from 1e-12 to 1e-7
    model.likelihood.variance = gpflow.base.Parameter(
        noise_variance, transform=gpflow.utilities.positive(lower=1e-7)
    )
