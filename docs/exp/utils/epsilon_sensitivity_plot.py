...
def plot_convergence_curve_for_cfg(cfg: dict, ax: Optional = None, plot_ylabel: bool = True,
                                   ...
                                   multi_panel_plot = None, move_up=0.15, box_scaling=0.8,
                                   uncertainty_quantification: bool = False, move_right=0.0,
                                   set_y_label_coords = None):
    ...
    color_idx = 0
    color_list = ["C0", "C1", "C2", "C3", "C4", "C5", "C6", "C7", "C8", "C9"] * 10 # Not use for color blindness
    ...
    for acq_exp_label, _ in cfg["acq"].items():
        # print(acq_exp_label)
        acq_exp_label_for_plot = acq_exp_label if acq_exp_label != "PF2ES_epsilon0.05" else '{PF}$^2$ES-$\epsilon(5\%)$'
        if acq_exp_label == 'PF2ES_origin':
            acq_exp_label_for_plot = '{PF}$^2$ES-$\epsilon(0\%)$'
        if acq_exp_label == 'PF2ES_003':
            acq_exp_label_for_plot = '{PF}$^2$ES-$\epsilon(3\%)$'
        if acq_exp_label == 'PF2ES_004':
            acq_exp_label_for_plot = '{PF}$^2$ES-$\epsilon(4\%)$'
        if acq_exp_label == "PF2ES_001":
            acq_exp_label_for_plot = '{PF}$^2$ES-$\epsilon(1\%)$'
        if acq_exp_label == "PF2ES_010":
            acq_exp_label_for_plot = '{PF}$^2$ES-$\epsilon(10\%)$'

        if uncertainty_quantification is False:
            x_space = 1 if 'x_space' not in cfg else cfg['x_space']
            # TODO: Support uncertainty calibration
    ...
    if set_y_label_coords is not None:
        ax.yaxis.set_label_coords(*set_y_label_coords)
...
vlmop2_epsilon_sensitivity_check_out_of_sample_log_hv_diff_exp_config = {
    "pb_name": "VLMOP2",
...
vlmop2_epsilon_sensitivity_check_in_sample_log_hv_diff_exp_config = {
    "pb_name": "VLMOP2",
...
if __name__ == '__main__':
    ...
    cfgs = [vlmop2_epsilon_sensitivity_check_out_of_sample_log_hv_diff_exp_config,
            ...
            cbranincurrin_epsilon_sensitivity_check_in_sample_log_hv_diff_exp_config,
            cbranincurrin_pf_uc_exp_config]
    fig, axis = plt.subplots(figsize=(10, 5.5), nrows=2, ncols=3)
    plot_convergence_curve_for_cfg(cfgs[0], ax=axis[0, 0], plot_ylabel=True, plot_xlabel=False, move_up=0.3, box_scaling=0.8)
    plot_convergence_curve_for_cfg(cfgs[1], ax=axis[0, 1], plot_ylabel=False, plot_xlabel=False, move_up=0.3, box_scaling=0.8)
    plot_convergence_curve_for_cfg(cfgs[2], ax=axis[0, 2], plot_ylabel=False, plot_xlabel=False, move_up=0.3, move_right=0.2,
                                   box_scaling=0.8, uncertainty_quantification=True)
    plot_convergence_curve_for_cfg(cfgs[3], ax=axis[1, 0], plot_ylabel=True, plot_title=False, move_up=0.4, box_scaling=0.8,
                                   set_y_label_coords=(-0.19, 0.5))
    plot_convergence_curve_for_cfg(cfgs[4], ax=axis[1, 1], plot_ylabel=False, plot_title=False, move_up=0.4, box_scaling=0.8)
    plot_convergence_curve_for_cfg(cfgs[5], ax=axis[1, 2], plot_ylabel=False, plot_title=False, move_up=0.4, move_right=0.2,
                                   box_scaling=0.8, uncertainty_quantification=True)
    axis[1, 0].text(-20, 0.5, 'log Hypervolume Difference', rotation=90, fontsize=15)
    axis[1, 0].text(97.5, 3.0, 'Hypervolume Indicator', rotation=90, fontsize=15)
    handles, labels = axis[0, 2].get_legend_handles_labels()
    fig.legend(handles, labels, loc='lower center', bbox_to_anchor=(0.5, 0.0), fancybox=False, ncol=4, fontsize=12)
    # plt.show(block=True)
    plt.savefig('epsilon_sensitivity_analyss.png', dpi=300)