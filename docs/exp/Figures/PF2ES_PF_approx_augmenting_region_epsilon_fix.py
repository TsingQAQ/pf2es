...
axis[0].plot([0.1, 0.1], [0.1, 0.8], color='k', linewidth=frame_linewidth, linestyle='--', zorder=0)
...
# axis[0].plot([0.1, 0.1], [0.8, 0.9], color='k', linewidth=frame_linewidth, zorder=0)
...
# TODO:
axis[0].scatter(np.asarray(obs)[:, 0] + 0.1, np.asarray(obs)[:, 1] + 0.1, s=100, marker='*', c='tab:green',
                edgecolors='k', zorder=100, label='Shifted Pareto Frontier')

modified_lb = np.array([[0.1, f(0.3) + 0.1], [0.3 + 0.1, f(0.5)+ 0.1], [0.5 + 0.1, f(0.7)+ 0.1], [0.7 + 0.1, 0.1]])
modified_ub = np.array([[0.3 + 0.1, 0.9], [0.5 + 0.1, 0.9], [0.7 + 0.1, 0.9], [0.9, 0.9]])
for i in range(lb.shape[0]):
    rect = patches.Rectangle(modified_lb[i], (modified_ub - modified_lb)[i, 0], (modified_ub - modified_lb)[i, 1],
                             linewidth=1, edgecolor='k', facecolor='#ffe699', alpha=1.0)
    axis[0].add_patch(rect)

axis[0].annotate('', xy=np.asarray(obs[0]) + 0.09,
                 xycoords='data',
                 xytext=obs[0],
                 arrowprops=dict(ls='--', linestyle='dashed', width=0.1, headwidth=5, headlength=5, color='k')
                 )
axis[0].annotate('', xy=np.asarray(obs[1]) + 0.09,
                 xycoords='data',
                 xytext=obs[1],
                 arrowprops=dict(ls='--', linestyle='dashed', width=0.1, headwidth=5, headlength=5, color='k')
                 )
axis[0].annotate('', xy=np.asarray(obs[2]) + 0.09,
                 xycoords='data',
                 xytext=obs[2],
                 arrowprops=dict(ls='--', linestyle='dashed', width=0.1, headwidth=5, headlength=5, color='k')
                 )
...
axis[0].text(0.15, 0.822, r'$\tilde{A}_1(\mathcal{F}_{\varepsilon})$', fontsize=tx_ftz - 5)
axis[0].text(0.428, 0.822, r'$\tilde{A}_2(\mathcal{F}_{\varepsilon})$', fontsize=tx_ftz - 5)
axis[0].text(0.619, 0.822, r'$\tilde{A}_3(\mathcal{F}_{\varepsilon})$', fontsize=tx_ftz - 5)
axis[0].text(0.816, 0.822, r'$\tilde{A}_4(\mathcal{F}_{\varepsilon})$', fontsize=tx_ftz - 5)
axis[0].text(0.25, 0.25, r'$\tilde{\overline{A}}(\mathcal{F}_{\varepsilon})$', fontsize=tx_ftz - 5)
...
builder = PF2ES(vlmop2_search_space, moo_solver='nsga2', sample_pf_num=5, remove_augmenting_region=False,
                population_size_for_approx_pf_searching=5, pareto_epsilon=0.04, remove_log=False,
...
res = minimize(lambda x: -np.squeeze(acq_f(np.atleast_2d(x)).numpy()), np.array([0.3, 0.3]), bounds=[(-2, 2), (-2, 2)])
...
#                  box.width, box.height * 0.85])
axis[2].set_position([box.x0 -  box.width * 1.2 , box.y0 + box.height * 0.2,
                 box.width, box.height * 0.85])
...
plt.savefig('epsilon_approach_Greedy_Issue_lgd.png', dpi=1000)
# plt.show()

# print(builder._pareto_epsilon)
# print(tf.sqrt(var1))
# print(tf.sqrt(var2))
